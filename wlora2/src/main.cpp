...
#define LORAWAN_APP_TX_DUTYCYCLE 1000 /**< Defines the application data transmission duty cycle. 10s, value in [ms]. */
...
#define V3_RADIO_DIO_1 14
#define V3_RADIO_NSS 8
#define V3_RADIO_RESET 12
#define V3_RADIO_BUSY 13

#define V3_LORA_CLK 9
#define V3_LORA_MISO 11
#define V3_LORA_MOSI 10

#ifdef ESP32
// ESP32 - SX126x pin configuration
int PIN_LORA_RESET = V3_RADIO_RESET; // LORA RESET
int PIN_LORA_NSS = V3_RADIO_NSS;	 // LORA SPI CS
int PIN_LORA_SCLK = V3_LORA_CLK;	 // LORA SPI CLK
int PIN_LORA_MISO = V3_LORA_MISO;	 // LORA SPI MISO
int PIN_LORA_DIO_1 = V3_RADIO_DIO_1; // LORA DIO_1
int PIN_LORA_BUSY = V3_RADIO_BUSY;	 // LORA SPI BUSY
int PIN_LORA_MOSI = V3_LORA_MOSI;	 // LORA SPI MOSI
...
void setup()
{
	...
	u8g2_print(u8g2, 10, 45, "AUS", FONT_28, true);
	...
	// Initialize LoRaWan
	err_code = lmh_init(&lora_callbacks, lora_param_init, false, CLASS_A, LORAMAC_REGION_EU868);
	if (err_code != 0)
	{
...
static uint8_t msg = 0x00;
void loop()
{
	// int btn_state0 = digitalRead(USER_BUTTON);
	// delay(100);
	// int btn_state1 = digitalRead(USER_BUTTON);

	// if (!btn_state0 && (btn_state0 == btn_state1))
	// {
	// 	uint32_t i = 0;
	// 	m_lora_app_data.port = LORAWAN_APP_PORT;
	// 	Serial.printf("Sending %d\n", msg % 2);
	// 	m_lora_app_data.buffer[i++] = (msg++) % 2;
...
static void send_lora_frame(void)
{
	...
	skip_count++;
	if (previous_state != btn_state || skip_count > 5)
	{
		skip_count = 0;
		if (previous_state == btn_state)
		{
			repeat_count++;
			if (repeat_count > 3)
			{
		...
		if (repeat_count < 3)
		{
			previous_state = btn_state;
			if (!btn_state)
			{
				u8g2_print(u8g2, 10, 45, "EIN", FONT_28, true);
				u8g2_print(u8g2, 10, 64, VER, FONT_11, false);
				einaus = 1;
			}
			else
			...
			m_lora_app_data.port = LORAWAN_APP_PORT;
			Serial.printf("Sending %d\n", einaus);
			m_lora_app_data.buffer[i++] = einaus;
			m_lora_app_data.buffsize = i;
			lmh_error_status error = lmh_send(&m_lora_app_data, LMH_UNCONFIRMED_MSG);
			Serial.printf("  lmh_send result %d - %ld ms - %d\n", error, millis(), repeat_count);
		}
	}
}
